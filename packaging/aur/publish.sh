#!/usr/bin/env bash
# Publish / update fat on the AUR (local helper).
# Usage:
#   ./packaging/aur/publish.sh           # push current packaging/aur
#   ./packaging/aur/publish.sh 0.1.1     # bump first, then push
#
...
echo "==> publishing fat $VER to AUR"
...
if git diff --cached --quiet; then
  ...
  git commit -m "fat ${VER}"
fi

echo "==> pushing to AUR…"
git push -u origin HEAD:master
echo "==> done → https://aur.archlinux.org/packages/fat"
